...
import {koa as klb} from "../src/index";
...
describe("koa middleware", () => {
    let logger: klb.MiddlewareReturnType["logger"];
    let mw: klb.MiddlewareReturnType["mw"];

    before(async () => {
        ({logger, mw} = await klb.middleware({logName: LOG_NAME, level: "info"}));
    });
